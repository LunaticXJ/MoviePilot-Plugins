...
class CloudStrmCompanion3(_PluginBase):
    # 插件名称
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "cloudstrmCompanion3_"
    # 加载顺序
    plugin_order = 3
    # 可使用的用户级别
    auth_level = 1
    ...
    def scan(self, event: Event = None):
        ...
        if event:
            event_data = event.event_data
            if not event_data or event_data.get("action") != "cloudStrmCompanion3":
                return
    ...
    def get_command() -> List[Dict[str, Any]]:
        ...
        return [
            {
                "cmd": "/cloud_strm_companion3",
                "event": EventType.PluginAction,
                "desc": "云盘Strm助手同步",
                "category": "",
                "data": {
                    "action": "CloudStrmCompanio3"
                }
            },
            {
                "cmd": "/strm",
